...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        # ADD CODE HERE
        beta = (motion[1]/self.length)*tan(motion[0]) # turning angle
        x_new = y_new = theta_new = 0.0
        if abs(beta) > 0.001:
            radius = motion[1]/beta # turning radius
            cx = self.x - radius * sin(self.orientation)
            cy = self.y + radius * cos(self.orientation)

            # global coordinates of robot
            theta_new = (self.orientation + beta) % (2 * pi)
            x_new = cx + radius * sin(theta_new)
            y_new = cy - radius * cos(theta_new)
        else: # straight motion
            x_new = self.x + motion[1]*cos(self.orientation)
            y_new = self.y + motion[1]*sin(self.orientation)
            theta_new = (self.orientation + beta) % (2 * pi)
