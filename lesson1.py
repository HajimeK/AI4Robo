...
# n localize takes the following arguments:
#
# colors:
...
def move(p, U, p_move):
    ...
    for i in range(len(p)):
        for j in range(len(p[0])):
            q[i][j] = p_move * p[(i-U[0]) % len(p)][(j-U[1]) % len(p[0])] + (1-p_move) *p[i][j]
            #print i, j, q[i][j]
